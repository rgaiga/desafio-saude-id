...
export class OrdersRepository {
    ...
    async getOrderById(productId: string): Promise<Order> {
        try {
            const response = (await this.redis.call('JSON.GET', `Order:${productId}`)) as string;
    ...
    async createOrder(orderId: string, body: CreateOrderDTO): Promise<Order> {
        try {
            await this.redis.call('JSON.SET', `Order:${orderId}`, '.', JSON.stringify(body));

            const response = (await this.redis.call('JSON.GET', `Order:${orderId}`)) as string;
