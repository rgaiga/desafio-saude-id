...
import { OrderNotFoundException } from '../../http-exceptions/order-not-found.http-exception';
...
export class OrdersRepository {
    ...
    async createOrder(body: CreateOrderDTO): Promise<Order | any> {
        const user = await this.userEntityRepository.findOneBy({ id: body.user_id });
        ...
        for (let i = 0; i < body.products.length; i++) {
            ...
            const product = await this.producEntityRepository.findOneBy({ id: product_id });
            ...
            totalPrice += body.products[i].unit_price * body.products[i].quantity;
        }
        ...
        const order = await this.orderEntityRepository.save(body);
    ...
    async updateOrder(orderId: string, body: UpdateOrderDTO): Promise<Order> {
        const order = await this.orderEntityRepository.findOneBy({ id: orderId });
